...
class DefaultChooser(object):
    ...
    def __init__(self, options):
        ...
        self.design_size = options.get('design-size', DEFAULT_NUMDESIGN)
    ...
    def fit(self, task_group, hypers=None, options=None):
        ...
        with open('/Users/mb/repos/pipeline-workspace/ansible-bigdata/grid.txt', 'a') as filehandle:
            for listitem in self.grid:
                filehandle.write('%s\n' % str(listitem))
        ...
        print(self.grid[self.design_index])
        ...
        for task_name, task in task_group.tasks.iteritems():
            ...
            if task.valid_values.shape[0] >= self.design_size:
    ...
    def suggest(self):
        ...
        if self.objective['inputs'].shape[0] < self.design_size:
            suggestion = self.task_group.from_unit(self.grid[self.design_index+2])
            #print(suggestion)
            sys.stderr.write("\nSuggestion:     ")
        ...
        # sys.stderr.write(current_best)
        # sys.stderr.write(current_best_location)
    ...
    def best(self):
        ...
        self.best_value = best_observed_value
        self.best_location = best_observed_location
        self.best_jobid = np.argmin(vals)
