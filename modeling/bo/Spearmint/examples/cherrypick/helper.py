...
def run_benchmark(config_file, parent_dir = 'configs/'):
    print "Provision machines and run the benchmark"
    original_cwd = os.getcwd()
    os.chdir(os.getcwd()+"/../")

    cmd = "python provision.py " + parent_dir + config_file + " " + configs.benchmark
    run_cmd(cmd)
...
def create_json(template, filename, params, framework, parent_dir = '../configs/'):
    print(os.getcwd())
    ...
    for key in params.keys():
        if key == "number":
            temp[framework]["number"] = params["number"]
        if key == "type":
            temp[framework]["type"] = params["type"]
            temp[framework]["memory"] = str(configs.memory[params["type"]]) + 'G'
    print(temp)
